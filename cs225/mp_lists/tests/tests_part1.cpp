...
TEST_CASE("List::split images", "[weight=5][part=1]") {
    ...
    for (unsigned i = 0; i < in.width(); i++) {
        for (unsigned j = 0; j < in.height(); j++) {
            ...
            // a++;
                        ...
                        // }    
                        // std::cout <<b << std::endl;
                        // vector<HSLAPixel> fake(list1.begin(), list1.end());
    ...
    for (unsigned i = 0; i < im1vect.size(); i++) {
        ...
        if (y == 239) { x++; }
                                ...
                                // for (unsigned i = 0; i < out1.width(); i++) {
                                //     for (unsigned j = 0; j < out1.height(); j++) {
                                //         if ( out1.getPixel(i, j) != expected_1.getPixel(i, j) )
