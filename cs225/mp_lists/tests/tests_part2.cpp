...
TEST_CASE("List::reverse", "[weight=5][part=2]") {
    ...
    // for (unsigned i = 0; i < 20; i++) { lista.insertFront(i); }
    // lista.reverse();
  ...
  PNG out = listToImage(list, in.width(), in.height());
                            ...
                            //         if ( out.getPixel(i, j) != expected.getPixel(i, j) )
...
TEST_CASE("List::merge", "[weight=10][part=2][valgrind]") {
    ...
    // List<int> lista;
    // for (unsigned i = 0; i < 10; i++) { lista.insertBack(i); }
    // lista.insertBack(0);
        // lista.insertBack(0);
        // lista.insertBack(0);
    ...
    // List<int> listb;
    // // for (unsigned i = 1; i < 10; i++) { listb.insertBack(i); }
    //     listb.insertBack(0);
    //     // listb.insertBack(0);
    //     // listb.insertBack(0);
    // stringstream t;
...
TEST_CASE("List::sort simple #1", "[weight=2][part=2][valgrind]") {
    ...
    list.sort();
                        // int k = 0;   
                        // for (auto it = list.head_; it->next != NULL; it = it->next) {
                        // // for (auto it = img_srt.begin(); it != img_srt.end(); it++) {
                        //     std::cout  << std::endl;
                        //     std::cout <<k <<": "<<it->data<< std::endl;
                        //     k++;
...
TEST_CASE("List::sort #2", "[weight=5][part=2]") {
  ...
  REQUIRE( b3 == expected );
                        ...
                        // int k = 0;   
                        // for (auto it = img_srt.head_; it->next != NULL; it = it->next) {
                        // // for (auto it = img_srt.begin(); it != img_srt.end(); it++) {
                        //     std::cout  << std::endl;
                        //     std::cout <<k <<": "<<it->data<< std::endl;
                        //     k++;
